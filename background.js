chrome.runtime.onInstalled.addListener(() => {
  const bookmarksToAdd = [
    ...
    { title: "Login", url: "javascript:(function(){ const wait = ms => new Promise(r => setTimeout(r, ms)); (async function(){ document.getElementById('alogin')?.click(); await wait(500); document.getElementById('contractorbtn')?.click(); })();})();" },
    ...
    { title: "🎯 Select Item", url: "javascript:(function(){const d=document.querySelector('#ddlitemnumber');if(!d)return;d.scrollIntoView({behavior:'smooth',block:'center'});d.style.border='3px solid red';setTimeout(()=>d.style.border='',2000);d.focus();})();" },
    ...
    { title: "📁 Choose Excel", url: "javascript:(function(){const wait=ms=>new Promise(r=>setTimeout(r,ms));(async function(){document.querySelector('button[data-bs-target=\"#MyPopup\"]')?.click();await wait(1000);document.getElementById('FileUploadexcel')?.click();})();})();" },
